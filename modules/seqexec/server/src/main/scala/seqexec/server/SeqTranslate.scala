...
class SeqTranslate(site: Site, systems: Systems[IO], settings: TranslateSettings) extends SequenceActions {
  ...
  private def step(obsId: Observation.Id, i: StepId, config: Config, nextToRun: StepId,
    ...
    def buildStep(
      ...
      val initialStepExecutions: List[List[Action[IO]]] =
        (i === 0).option {
          inst.initialStepAction(stepType,
            systems.odb.sequenceStart(obsId, "")
      ...
      def rest(ctx: HeaderExtraData): List[List[Action[IO]]] =
        inst.observeActions(stepType,
          observe(systems, config, obsId, inst, sys.filterNot(inst.equals), headers)(ctx))
