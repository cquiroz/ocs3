...
trait SequenceActions {
  ...
  private def sendObservationAborted[F[_]: LiftIO: Monad](systems: Systems[F], obsId: Observation.Id, imageFileId: ImageFileId): SeqActionF[F, Unit] =
  ...
  private def sendDataStart[F[_]: LiftIO: Monad](systems: Systems[F], obsId: Observation.Id, imageFileId: ImageFileId, dataId: String): SeqActionF[F, Unit] =
  ...
  def observeActions[F[_]](
      result: Stream[F, Result[F]]
  ): List[List[Action[F]]] =
    List(
      List(Action(ActionType.Observe, result, Action.State(ActionState.Idle, Nil)))
    )
}

object SequenceActions extends SequenceActions
