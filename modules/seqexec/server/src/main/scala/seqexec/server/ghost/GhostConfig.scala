...
sealed trait GhostConfig {
  ...
  def fiberAgitatorConfig: Configuration =
    GhostConfig.fiberConfig(fiberAgitator)

  def configuration: Configuration = fiberAgitatorConfig |+| ifu1Config |+| ifu2Config
}
...
object GhostConfig {
  private[ghost] def ifuConfig(ifuNum: IFUNum,
    ...
    cfg("type", DemandType.DemandRADec) |+|
  ...
  private[ghost] def ifuPark(ifuNum: IFUNum): Configuration = {
    ...
    cfg("type", DemandType.DemandPark)
  }

  private[ghost] def fiberConfig(fa: FiberAgitator): Configuration =
    Configuration.single(GhostFiberAgitator.applyItem, fa)
