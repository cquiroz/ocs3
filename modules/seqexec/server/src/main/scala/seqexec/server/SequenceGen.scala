...
object SequenceGen {
  ...
  object StepGen {
    def generate[F[_]](stepGen: StepGen[F], ctx: HeaderExtraData): EngineStep[F] =
      stepGen match {
  ...
  final case class SkippedStepGen(override val id: StepId, override val config: CleanConfig)
