...
object StepsTable extends Columns {
  ...
  def updateScrollPosition(b: Backend, pos: JsNumber): Callback = {
    ...
    val modMod =
      b.setStateL(State.userModified)(IsModified).when_(hasScrolledBefore)
    ...
    val newTs = if (hasScrolledBefore) {
      (State.userModified.set(IsModified) >>> State.scrollPosition.set(pos))(
        b.state
      )
    } else {
    ...
    val scrolledWhileRunningMod = b
      .setStateL(State.scrollWhileRun)(true)
      .when_(
        b.props.sequenceState
          .exists(_.isRunning) && posDiff > 2 * (HeightWithOffsets + BreakpointLineHeight)
      )
    ...
    (posMod *>
      ...
      b.props.obsId
        .map(
          id =>
            SeqexecCircuit
              .dispatchCB(UpdateStepTableState(id, newTs.tableState))
        )
        .getOrEmpty)
  ...
  def initialState(p: Props): State =
    (State.tableState.set(p.tableState) >>> State.selected.set(p.selectedStep))(
      State.InitialState
    )
