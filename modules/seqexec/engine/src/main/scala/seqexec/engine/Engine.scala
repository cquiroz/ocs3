...
class Engine[F[_]: MonadError[?[_], Throwable]: Logger, D, U](stateL: Engine.State[F, D]) {
      ...
      Handle.fromStream[F, D, EventType](
        p.attempt.flatMap {
          case Right(r @ Result.OK(_))    =>
            Stream.eval(singleRunCompleted(c, r).pure[F])
          case Right(e @ Result.Error(_)) =>
            Stream.eval(singleRunFailed(c, e).pure[F])
          case Right(r)                   =>
            Stream.eval(singleRunFailed(c, Result.Error(s"Unhandled result for single run action: $r")).pure[F])
  ...
  : Stream[F, EventType] = t match {
    case (gen, i) =>
      gen.takeThrough {
        ...
        case Right(r@Result.OK(_))        => Stream.eval(completed(id, stepId, i, r).pure[F])
        case Right(r@Result.OKStopped(_)) => Stream.eval(stopCompleted(id, stepId, i, r).pure[F])
        case Right(r@Result.Partial(_))   => Stream.eval(partial(id, stepId, i, r).pure[F])
        case Right(e@Result.Error(_))     => Stream.eval(failed(id, i, e).pure[F])
        case Right(r@Result.Paused(_))    => Stream.eval(paused[F](id, i, r).pure[F])
        case Left(t: Throwable)           => Stream.raiseError[F](t)
      }
  }
  ...
  private def logError(e: Result.Error): HandleType[Unit] = error(e.errMsg.getOrElse(e.msg))
    ...
    /**
      ...
      */
    def info(msg: => String): HandleType[Unit] = pure((L.info(msg), None)).void

    /**
      * Log warning lifted into Handle.
      */
    def warning(msg: => String): HandleType[Unit] = pure((L.warn(msg), None)).void

    /**
      * Log debug lifted into Handle.
      */
    def debug(msg: => String): HandleType[Unit] = pure((L.debug(msg), None)).void

    /**
      * Log error lifted into Handle
      */
    def error(msg: => String): HandleType[Unit] = pure((L.error(msg), None)).void
  ...
  private def handleUserEvent(ue: UserEventType): HandleType[ResultType] = ue match {
    ...
    case LogInfo(msg)                  => info(msg) *> pure(UserCommandResponse(ue, Outcome.Ok, None))
